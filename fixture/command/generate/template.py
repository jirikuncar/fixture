...
class _TemplateRegistry(object):
    ...
    def register(self, template, default=False):
        ...
        if name in self.lookup:
            raise ValueError("cannot re-register %s with object %s" % (name,
                                                                       template))
...
class fixture(Template):
    ...
    class DataDef(Template.DataDef):
        ...
        def fset_to_attr(self, fset, fxt_class):
            ...
            return code_str("%s.%s.ref(%s)" % (fxt_class, fset.mk_key(),
                                               repr(fset.get_id_attr())))
...
class testtools(Template):
    ...
    class DataDef(Template.DataDef):
        ...
        def fset_to_attr(self, fset, fxt_class):
            return code_str("r.%s.%s.%s" % (fset.mk_var_name(), fset.mk_key(),
                                            fset.get_id_attr()))
